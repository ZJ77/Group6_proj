...
def evaluate(env, agent):
    ...
    for _ in range(TEST_EPISODES):
        ...
        for _ in range(MAX_STEP_PER_EPISODE):
            with torch.no_grad():
                actions = agent.get_action(obs, g, True)
            observation_new, _, done, info = env.step(actions)
...
def train(video_record=None):
    ...
    # create an agent
    agent = DDPG_AGENT(env, log_dir=logdir, writer=writer)
    ...
    for epoch in range(EPOCH_NUM):
        ...
        for i in range(EPISODES_PER_EPOCH): #
            ...
            for step in range(MAX_STEP_PER_EPISODE):
                ...
                episode_acs.append(ac) # TODO not use .copy
                episode_r.append([r])
...
def test(episodes=10, agent=None):
    ...
    if agent ==None:
        ...
        # create an agent:
        agent = DDPG_AGENT(env, log_dir=logdir)
        # load weights
    ...
    for _ in range(episodes):
        for step in range(MAX_STEP_PER_EPISODE):
            env.render()
            ac = agent.get_action(ob, d_g, True)
